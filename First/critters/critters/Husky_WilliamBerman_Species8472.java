...
public class Husky_WilliamBerman_Species8472 extends Critter {
	...
	private int count = 0;
	private int phaseOneEnd = 200;
	/* Phase One: pure upwelling
	   Phase Two: colonize the bottom and upwell later*/
	...
	public Husky_WilliamBerman_Species8472 () {
	...
	public Action getMove (CritterInfo info) {
		...
		//End the hibernation in the 8472, every time hibernationEnd is reached
			/*
			if (this.count >= this.hibernationEnd
				&& (this.what == status.HOLDFAST || this.what == status.DORMANT)) {
		...
		//Determine if this 8472 should go from holdfast to dormant
			if (this.count > this.phaseOneEnd) {
		...
		more so if there are more of them. */
			if ((this.what == status.UPWELLING
					&& (info.getFront() == Neighbor.WALL || info.getFront() == Neighbor.SAME))
				|| this.count <= this.phaseOneEnd) {
		...
		/* Homing tests: these 8472 turn towards adjacent enemies, trying to infect them.
		 ...
		 as the "else" ensures that only one of these options happens. */
			...
			else if (info.getBack() == Neighbor.OTHER && info.getFront() == Neighbor.EMPTY) { //If enemy B
				return Action.HOP; //Escape from the Neighbor.OTHER behind this 8472
			}
			
			else if (info.getFront() == Neighbor.OTHER) { //If enemy B, F
				return Action.INFECT;
			}
