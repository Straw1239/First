...
import static testcanvas.Particle.MIN_RADIUS;
import static testcanvas.Particle.RADIUS_DELTA;
...
public class CanvasParticleAnimater {
	...
	public static boolean allWithinBounds(double... args)
	{
		for(int i = 0; i < args.length; i++)
		{
			if(args[i] <= Particle.DC    || args[i] >= 1 - Particle.DC   ) return false;
		}
		return true;
	}
	...
	private void renderFrame() {
		...
		for (final Particle p : particles) {
			...
			if(p.rDC)
			{
				if(allWithinBounds(c.getBlue() + h.getRed(), c.getOpacity() + h.getOpacity(), c.getRed() + h.getRed(), c.getGreen() +h.getGreen()))
				{
					c = Color.color(c.getRed() + h.getRed(), c.getGreen() + h.getGreen() ,c.getBlue() + h.getBlue(), c.getOpacity() + h.getOpacity());
				}
				else
				{
					p.rDC = !p.rDC;
