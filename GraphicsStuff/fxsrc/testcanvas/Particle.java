...
public class Particle {
	 ...
	 */
	public static Random rand = new XRandom();
	public static final double DC = .2;
	public boolean rDC = true;
	public Color dC = Color.color(rand.nextDouble() * DC, rand.nextDouble() * DC, rand.nextDouble() * DC, rand.nextDouble() * DC);
	public Particle(final int canvasWidth, final int canvasHeight) {
		...
		x = Math.round(rand.nextDouble() * 50);
		if (rand.nextBoolean()) {
			x = canvasWidth - x;
		}
		y = Math.round(rand.nextDouble() * 50);
		if (rand.nextBoolean()) {
			y = canvasHeight - y;
		}

		// Select a random colour from our preselected range.
		colour = Color.color(rand.nextDouble(), rand.nextDouble(), rand.nextDouble());

		// Randomly set the x and y deltas to be a positive or negative value within the allowable range.
		dx = rand.nextInt(MAX_DELTA) - MAX_DELTA / 2;
		dy = rand.nextInt(MAX_DELTA) - MAX_DELTA / 2;

		// Randomly set the radius, maximum radius and radius deltas to be within the allowable range.
		r = rand.nextDouble() * MAX_RADIUS;
		maxR = rand.nextDouble() * MAX_RADIUS / 2 + 10;
		dr = rand.nextBoolean() ? RADIUS_DELTA : -RADIUS_DELTA;
	}
